...
This pipeline processes raw Reduced Representation Bisulfite Sequencing
data, running QC, trimming, alignment, and methylation calling tools.
It uses the Pypiper pipeline framework and can be easily parallelized
using Looper.
"""
...
if not args.paired_end and args.pdr:
	...
	pm.timestamp("### PDR (Proportion Discordant Reads) analysis")
	...
	if os.path.isfile(os.path.join(tools.scripts_dir, "extractPDR.pl")):
		...
		pm.timestamp("### PDR (Perl version by K.C.)")	
